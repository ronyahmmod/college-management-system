  ...
  const fetchSubjects = async () => {
    try {
      const response = await api.get(
        `/subjects?page=${currentPage}&limit=10&search=${search}&department=${departmentFilter}`
      );
  ...
  return (
    <div className="min-h-screen bg-gray-100 p-6">
      <div className="container mx-auto">
        <h2 className="text-2xl font-bold mb-6">সাবজেক্ট লিস্ট</h2>
        {error && <p className="text-red-500 mb-4">{error}</p>}
        <div className="mb-4 flex space-x-4">
          <input
            type="text"
            placeholder="সাবজেক্ট সার্চ করুন..."
            value={search}
        ...
        {subjects.length === 0 ? (
          <p className="text-gray-600">কোনো সাবজেক্ট পাওয়া যায়নি</p>
        ) : (
          <>
